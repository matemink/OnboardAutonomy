...
bool valid_observation(
    const domain::TargetObservation& observation,
    const double minimum_decision_margin
) {
    if (!observation.pose.has_value() ||
        observation.corrected_bits != 0 ||
    ...
    return std::isfinite(position.right_m) &&
        std::isfinite(position.down_m) &&
        std::isfinite(position.forward_m) &&
        position.forward_m > 0.0;
}

domain::CameraFramePosition smooth_position(
    const domain::CameraFramePosition& previous,
    const domain::CameraFramePosition& measurement,
    const double factor
) {
    const double retained = 1.0 - factor;
    return {
        .right_m = retained * previous.right_m +
            factor * measurement.right_m,
        .down_m = retained * previous.down_m +
            factor * measurement.down_m,
        .forward_m = retained * previous.forward_m +
            factor * measurement.forward_m,
    };
}
...
void TargetTracker::update(
    const std::span<const domain::TargetObservation> observations,
    ...
    const domain::TargetObservation* selected = nullptr;
    for (const auto& observation : observations) {
        ...
        if (target_id_.has_value() &&
            observation.id != *target_id_) {
            continue;
        }
        if (selected == nullptr ||
            observation.decision_margin >
                selected->decision_margin) {
...
TargetTrackSnapshot TargetTracker::snapshot(
    ...
    result.phase = consecutive_observations_ >=
            config_.required_consecutive_observations
...
bool TargetTracker::expired(const domain::TimePoint now) const {
    return last_observed_at_.has_value() &&
        now >= *last_observed_at_ &&
