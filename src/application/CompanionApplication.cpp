...
std::vector<std::uint8_t> encode_flight_action(
    ...
    case FlightAction::arm:
        return adapters::mavlink::encode_arm(request.vehicle_system_id,
            request.confirmation);
    case FlightAction::takeoff:
        return adapters::mavlink::encode_takeoff(request.vehicle_system_id,
            request.altitude_m,
            request.confirmation);
    case FlightAction::return_to_launch:
        return adapters::mavlink::encode_return_to_launch(
            ...
            request.confirmation);
    case FlightAction::land:
        return adapters::mavlink::encode_land(request.vehicle_system_id,
            request.confirmation);
    case FlightAction::landing_target:
...
class CompanionApplication::Impl {
  public:
    explicit Impl(ports::Transport& transport,
        ...
        : transport_(transport),
          ...
          decoder_{
              vehicle_state_,
              [this](const adapters::mavlink::CommandAck& acknowledgement,
                  ...
                  if (acknowledgement.target_component == 0 ||
                      ...
                      if (acknowledgement.command ==
                          ...
                          telemetry_before.phase ==
                              adapters::mavlink::TelemetrySetupPhase::
                                  configuring &&
              ...
              [this](const adapters::mavlink::MessageObservation& message,
                  const domain::TimePoint now) {
                  record_activity(LinkEventDirection::inbound,
              ...
              [this](const adapters::mavlink::ParameterValue& parameter,
                  const domain::TimePoint now) {
    ...
    void poll(const domain::TimePoint now) { poll(std::optional{now}); }

  private:
    domain::TimePoint poll_inputs(
        const std::optional<domain::TimePoint>& fixed_now) {
        const auto camera_now = fixed_now.value_or(domain::Clock::now());
    ...
    void update_vehicle_connection(const domain::VehicleSnapshot& vehicle,
        const domain::TimePoint now) {
    ...
    void send_companion_heartbeat(const domain::VehicleSnapshot& vehicle,
        const domain::TimePoint now) {
        if (now < next_heartbeat_) {
            return;
        }
        if (vehicle.connected && vehicle.system_id.has_value()) {
            const auto frame = adapters::mavlink::encode_companion_heartbeat(
    ...
    bool update_telemetry_setup(const domain::VehicleSnapshot& vehicle,
        const domain::TimePoint now) {
    ...
    void request_failsafe_parameter(const domain::VehicleSnapshot& vehicle,
        ...
        const bool sent =
            write_frame(adapters::mavlink::encode_parameter_request_read(
                            vehicle.system_id.value(),
                            name),
                now,
                "PARAM_REQUEST_READ",
    ...
    void request_vehicle_metadata(const domain::VehicleSnapshot& vehicle,
        const bool telemetry_ready,
        const domain::TimePoint now) {
        if (!telemetry_ready || !vehicle.system_id.has_value()) {
            return;
        ...
        if (!vehicle.autopilot_metadata.has_value() &&
            ...
            const bool sent = write_frame(
                adapters::mavlink::encode_autopilot_version_request(system_id),
                now,
                "COMMAND_LONG",
        ...
        const bool sent = write_frame(
            adapters::mavlink::encode_battery_arming_voltage_request(system_id),
            now,
            "PARAM_REQUEST_READ",
            "BATT_ARM_VOLT");
    ...
    void update_flight_automation(const domain::VehicleSnapshot& vehicle,
        const bool telemetry_ready,
        const domain::TimePoint now) {
  ...
  private:
    ...
    current_landing_target(const domain::TimePoint now) const {
        ...
        return domain::camera_to_body_frd(*vision->target_track.position,
            *camera_extrinsics_);
    }
    ...
    std::chrono::milliseconds elapsed_at(const domain::TimePoint now) {
        if (!started_at_.has_value()) {
            started_at_ = now;
        }
    ...
    void observe_vehicle(const domain::VehicleSnapshot& vehicle,
        const domain::TimePoint now) {
        if (!vehicle.connected) {
...
bool CompanionApplication::request_autonomy_start(const domain::TimePoint now) {
