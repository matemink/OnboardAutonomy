...
class GStreamerCameraSource final : public mission::ports::CameraSource {
  public:
    ...
    [[nodiscard]] std::optional<mission::ports::CameraFrame>
    ...
    [[nodiscard]] mission::ports::CameraSourceStatus status() const override {
        std::scoped_lock lock(state_mutex_);
        return status_;
    }
  ...
  private:
    ...
    void run_once(const std::stop_token& stop_token) {
        ...
        while (!stop_token.stop_requested()) {
            mission::ports::CameraFrame frame{
                .sequence = 0U,
                .width = config_.width,
        ...
        if (status().phase == mission::ports::CameraSourcePhase::failed) {
            return;
        }
    ...
    void publish(mission::ports::CameraFrame frame) {
        std::scoped_lock lock(state_mutex_);
        ...
        status_.phase = mission::ports::CameraSourcePhase::streaming;
        status_.error.clear();
    }

    void set_failure(std::string error) {
        std::scoped_lock lock(state_mutex_);
        status_.phase = mission::ports::CameraSourcePhase::failed;
    ...
    void set_reconnecting(std::string error) {
        std::scoped_lock lock(state_mutex_);
        latest_frame_.reset();
        status_.phase = mission::ports::CameraSourcePhase::reconnecting;
        status_.error = std::move(error);
    ...
    mission::ports::CameraSourceStatus status_;
...
std::unique_ptr<mission::ports::CameraSource> make_gstreamer_camera_source(
