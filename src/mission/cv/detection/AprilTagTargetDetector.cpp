...
class DetectionArray {
  public:
    explicit DetectionArray(zarray_t* detections) : detections_(detections) {}
    ...
    [[nodiscard]] zarray_t* get() const { return detections_; }
...
class Matrix {
  public:
    explicit Matrix(matd_t* matrix) : matrix_(matrix) {}

    ~Matrix() { matd_destroy(matrix_); }

    Matrix(const Matrix&) = delete;
    Matrix& operator=(const Matrix&) = delete;

    [[nodiscard]] matd_t* get() const { return matrix_; }
...
class Correspondences {
  public:
    Correspondences() : values_(zarray_create(sizeof(float[4]))) {
        if (values_ == nullptr) {
    ...
    [[nodiscard]] zarray_t* get() const { return values_; }

  private:
    zarray_t* values_;
};

std::optional<mission::TargetPose> estimate_pose(
    ...
    for (std::size_t index = 0; index < tag_corners.size(); ++index) {
        ...
        correspondences.add(tag_corners[index][0],
            tag_corners[index][1],
            point);
    }
    ...
    Matrix homography{
        homography_compute(correspondences.get(), HOMOGRAPHY_COMPUTE_FLAG_SVD)};
    ...
    for (std::size_t row = 0; row < 3U; ++row) {
        for (std::size_t column = 0; column < 3U; ++column) {
            pose.rotation_tag_to_camera[row * 3U + column] =
                MATD_EL(rotation.get(),
                    static_cast<unsigned int>(row),
                    static_cast<unsigned int>(column));
        }
    }

    const bool finite_position = std::isfinite(pose.position.right_m) &&
                                 std::isfinite(pose.position.down_m) &&
                                 std::isfinite(pose.position.forward_m);
    const bool finite_rotation =
        std::ranges::all_of(pose.rotation_tag_to_camera,
...
class AprilTagTargetDetector final : public mission::ports::TargetDetector {
  public:
    explicit AprilTagTargetDetector(const AprilTagDetectorConfig& config)
        : pose_config_(config.pose) {
        if (config.worker_threads == 0U ||
            config.worker_threads >
                static_cast<std::uint32_t>(std::numeric_limits<int>::max()) ||
            config.quad_decimate < 1.0 || config.corrected_bits < 0 ||
            config.corrected_bits > 2) {
            throw std::invalid_argument(
                "invalid AprilTag detector configuration");
        }
        if (pose_config_.has_value()) {
            mission::validate_camera_calibration(pose_config_->calibration);
        ...
        if (family_ == nullptr || detector_ == nullptr) {
            ...
            throw std::runtime_error("unable to create AprilTag detector");
        }

        detector_->nthreads = static_cast<int>(config.worker_threads);
        detector_->quad_decimate = static_cast<float>(config.quad_decimate);
        detector_->quad_sigma = 0.0F;
        detector_->refine_edges = config.refine_edges;
        detector_->debug = false;
        apriltag_detector_add_family_bits(detector_,
            family_,
    ...
    [[nodiscard]] mission::TargetDetectionBatch detect(
        ...
        if (pose_config_.has_value() &&
            (frame.width != pose_config_->calibration.image_width ||
                frame.height != pose_config_->calibration.image_height)) {
            throw std::invalid_argument(
        ...
        image_u8_t image{
            ...
            .buf = const_cast<std::uint8_t*>(frame.yuv420.data()),
        };

        const auto processing_started = std::chrono::steady_clock::now();
        DetectionArray detections{apriltag_detector_detect(detector_, &image)};
        const auto detected_at = std::chrono::system_clock::now();
        ...
        for (int index = 0; index < count; ++index) {
            ...
            std::array<mission::ImagePoint, 4> corners{};
            ...
            targets.push_back({
                .id = detection->id,
                .family = detection->family != nullptr &&
...
std::unique_ptr<mission::ports::TargetDetector> make_apriltag_target_detector(
